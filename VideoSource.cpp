...
int VideoState::queue_picture_2(AVFrame *pFrame)
{
	uint8_t *buffer = NULL;
	int numBytes;
	...
	SaveFrame(pFrameRGB, codecCtx->width, codecCtx->height, ++frame_no);
	return 0;
}
...
void VideoState::video_thread_loop(VideoState *self)
{
	...
	while (!self->video_quit)
	{
		...
		if (frameFinished)
		{
			...
			//ret = self->queue_picture_2(pFrame);
			if (ret < 0)
				break;
		}
	}
...
void VideoSource::start_airplay()
{
	...
	//int ret = XinDawn_StartMediaServer("XinDawn1", ".\\Debug\\airmediaserve.dll", 800, 600, "000000000", &ao);
...
void gdiscreen(HWND hwnd) {
	...
	GdiplusStartup(&gdiplusToken, &gdiplusStartupInput, NULL);
	{
		...
		height = RectHeight(rc);//rc.bottom - rc.top;  //change this to whatever size you want to resize to
		width = RectWidth(rc);//rc.right - rc.left;
...
int main(int argc, char *argv[])
{
	...
	HANDLE hEnent = CreateEvent(NULL, FALSE, FALSE, L"Global\\captureimage");
	
	while (true)
	{
		//Sleep(1000);
		if (WaitForSingleObject(hEnent, INFINITE) == WAIT_OBJECT_0)
		{
			HWND hwnd = FindWindow(NULL, L"XinDawn AirPlay Mirroring SDK");
			gdiscreen(hwnd);
		}
